...
public class DiskGraphCreationOptionTests
{
  ...
  public void Initializer_InvalidMeridianCount_ThrowsArgumentException(int invalidValue) =>
    Assert.That(() => new DiskGraphCreationOption<int, int> { MeridianCount = invalidValue }, Throws.ArgumentException);

  [TestCase(0)]
  [TestCase(-1)]
  public void Initializer_InvalidRingCount_ThrowsArgumentException(int invalidValue) =>
    Assert.That(() => new DiskGraphCreationOption<int, int> { RingCount = invalidValue }, Throws.ArgumentException);
}