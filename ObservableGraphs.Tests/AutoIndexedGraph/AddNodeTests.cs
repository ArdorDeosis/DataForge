...
internal class AddNodeTests
{
  ...
  public void AddNode_Node_HasExpectedIndex()
  {
    ...
    var graphs = new[]
    {
      new ObservableAutoIndexedGraph<int, int, int>(new IncrementalIndexProvider<int, int>(index)),
      new ObservableAutoIndexedGraph<int, int, int>(_ => index),
      new ObservableAutoIndexedGraph<int, int, int>(new IncrementalIndexProvider<int, int>(index),
        () => EqualityComparer<int>.Default),
      new ObservableAutoIndexedGraph<int, int, int>(_ => index, () => EqualityComparer<int>.Default)
    };
  ...
  public void AddNode_InvalidIndex_ThrowsInvalidOperationException()
  {
    // ARRANGE
    const int data = 0xC0FFEE;
    var graph = new ObservableAutoIndexedGraph<int, int, int>(new StatelessIndexProvider<int, int>(data => data));
    graph.AddNode(data);
